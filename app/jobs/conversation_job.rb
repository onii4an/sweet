class ConversationJob < ApplicationJob
  ...
  def create(boy_id, girl_id, conversation_id)
    ...
    ActionCable.server.broadcast "user_#{boy_id}", girl_id: girl_id, action: :create, res: ApplicationController.renderer.render(partial: 'notification/notification', object: conversation_boy)
    ActionCable.server.broadcast "user_#{girl_id}", boy_id: boy_id, action: :create, res: ApplicationController.renderer.render(partial: 'notification/notification', object: conversation_girl)
